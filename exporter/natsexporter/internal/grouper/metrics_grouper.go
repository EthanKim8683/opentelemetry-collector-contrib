	...
	for _, srcResourceMetrics := range srcMetrics.ResourceMetrics().All() {
		...
		for _, srcScopeMetrics := range srcResourceMetrics.ScopeMetrics().All() {
			...
			for _, srcMetric := range srcMetrics.All() {
				...
				if !ok {
					errs = multierr.Append(errs, fmt.Errorf("metrics subject is not a string: %v", subjectAsAny))
					continue
				}
...
func NewMetricsGrouper(subjectExpression string, telemetrySettings component.TelemetrySettings) (Grouper[pmetric.Metrics], error) {
